...
import TopHeader from "./topHeader";
import Navbar from "./navbar";
import MobileMenu from "../layout/mobileMenu";   // adjust path if needed

export default function HeaderWrapper() {
  const [atTop, setAtTop] = useState(true);
  const [mobileOpen, setMobileOpen] = useState(false);   // ← moved here
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${
          atTop ? "translate-y-0 opacity-100" : "-translate-y-full pointer-events-none"
        }`}
      >
        <TopHeader />
      </div>

      {/* Navbar – now receives mobileOpen setters as props */}
      <div
        className={`fixed left-0 right-0 z-40 transition-all duration-500 ease-out ${
          atTop
            ? "top-[theme(spacing.14)] bg-black/90 backdrop-blur-3xl"   // ≈56px, adjust if needed
            : "top-0 bg-black/95 backdrop-blur-3xl shadow-2xl shadow-black/60"
        }`}
      >
        <Navbar
      ...
      {/* Mobile Menu – rendered at root level of HeaderWrapper → avoids stacking issues */}
      <MobileMenu
        isOpen={mobileOpen}
      ...
      <div
        className={`transition-all duration-500 ease-in-out ${
          atTop ? "h-28 lg:h-32" : "h-20 lg:h-24"
        }`}
      />
    </>
  );
